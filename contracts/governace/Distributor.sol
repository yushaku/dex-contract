...
contract RewardsDistributor is IRewardsDistributor {
  ...
  constructor(address _veToken) {
    ...
    address _token = IVotingEscrow(_veToken).token();
    token = _token;
    voting_escrow = _veToken;
    depositor = msg.sender;
    owner = msg.sender;

    require(IERC20(_token).approve(_veToken, type(uint).max));
  }
  ...
  function claim(uint _tokenId) external returns (uint) {
    ...
    if (amount != 0) {
      ...
      IVotingEscrow.LockedBalance memory _locked = IVotingEscrow(voting_escrow).locked(
        _tokenId
      );
  ...
  function claim_many(uint[] memory _tokenIds) external returns (bool) {
    ...
    for (uint i = 0; i < _tokenIds.length; i++) {
      ...
      if (amount != 0) {
        // if locked.end then send directly
        IVotingEscrow.LockedBalance memory _locked = IVotingEscrow(_voting_escrow).locked(
          _tokenId
        );
        if (_locked.end < block.timestamp) {
